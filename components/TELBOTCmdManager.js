...
class TELBOTCmdsParser {
  constructor () {
    console.log("|TELBOT] TELBOTCmdsManager")
  }

  parseCommand(that, msg) {
    const createHandler = (msg, args) => {
      var callbacks = {
        reply: that.reply.bind(that),
        ask: that.ask.bind(that),
        say: that.say.bind(that)
      }
    ...
    if (matchedCommands.length == 1) {
      ...
      if (that.config.commandAllowed.hasOwnProperty(c.command)) {
        ...
        if (Array.isArray(allowedUser) && allowedUser.length > 0) {
          if (!allowedUser.includes(msg.from.username)) {
            ...
            var text = that.translate("TELBOT_NOT_ALLOWED_COMMAND")
      ...
      //0 or multi
      var handler = createHandler(msg, null)
      var text = that.translate("TELBOT_NOT_REGISTERED_COMMAND")
      if (matchedCommands.length > 1) {
        text = that.translate("TELBOT_FOUND_SEVERAL_COMMANDS")
